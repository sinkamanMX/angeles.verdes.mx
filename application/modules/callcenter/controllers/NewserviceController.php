    	...
    	try{
			...
			if(isset($this->_dataIn['optReg'])){
				...
				if($errors==0){
					...
					$sUassign 	= $cHorarios->getUserAssign($this->_dataIn['inputDate'],$this->_dataIn['inputhorario']);
					if($sUassign!="" && $sUassign>-1){
						$this->_dataIn['uAssign'] = $sUassign;
    ...
    public function gethorariosAction(){
    	try{
			...
			if(isset($this->_dataIn['dateID']) && $this->_dataIn['dateID']!=""){
				...
				if($sucursales!=""){					
					$cHorarios 		= new My_Model_Horarios();
					$dataHorarios	= $cHorarios->getHorarios($sucursales,$this->_dataIn['dateID']);
					if(count($dataHorarios)>0){
						$result = $cFunctions->cboHorarios($dataHorarios,$this->_dataIn['dateID']);	
						if($result==""){
