...
class My_Model_Perfiles extends My_Db_Table
{
	...
	public function getModulesByProfile($idObject,$idEmpresa){
		$result= Array();
		$this->query("SET NAMES utf8",false); 		
    	$sql ="SELECT M.ID_MODULO AS ID, N.DESCRIPCION AS MENU, M.DESCRIPCION AS N_MODULE, IF(P.ID_MODULO IS NULL ,'0' ,'1') AS ASIGNADO, IF(P.INICIO IS NULL,'0',P.INICIO) AS INICIO
				FROM  MODULOS M 
				INNER JOIN MENU    N ON M.ID_MENU 	 = N.ID_MENU
				 LEFT JOIN MODULOS_PERFIL P ON M.ID_MODULO = P.ID_MODULO AND P.ID_PERFIL = $idObject   AND P.`INICIO` IS NOT NULL
				WHERE M.ACTIVO = 1
